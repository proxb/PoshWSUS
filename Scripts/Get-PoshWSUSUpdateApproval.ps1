function Get-PoshWSUSUpdateApproval {
    <#  
    .SYNOPSIS  
    ...
    .NOTES  
        Name: Get-PoshWSUSUpdateApproval
        Author: Boe Prox
    ...
    .EXAMPLE   
    Get-PoshWSUSUpdateApproval -Update 2546951 
    ...
    .EXAMPLE
    $updatescope = New-PoshWSUSUpdateScope -FromArrivalDate "11/01/2011"
    Get-PoshWSUSUpdateApproval -UpdateScope $updatescope
    ...
    Process {
        If ($PSBoundParameters['Update']) {
            If ($update -is [Microsoft.UpdateServices.Internal.BaseApi.Update]) {
                ...
                $patches = Get-PoshWSUSUpdate -update $Update
            }
        } ElseIf ($PSBoundParameters['UpdateScope']) {
            Write-Verbose ("Gathering update data via update scope")
            $patches = Get-PoshWSUSUpdate -UpdateScope $UpdateScope
        } Else {
            #Create default update scope to use for update search
            $patches = Get-PoshWSUSUpdate -UpdateScope (New-PoshWSUSUpdateScope)
        }
